...
if (!dir.exists("renv")) {renv::init(bare = TRUE)} else {renv::activate()}
...
# Instalamos los paquetes de CRAN:
paquetes_cran <- c("tidyverse", "data.table", "readr", "readxl", "ggplot2", "ROCR")
for (p in paquetes_cran) {if (!requireNamespace(p, quietly = TRUE)) install.packages(p)
  suppressPackageStartupMessages(library(p, character.only = TRUE))}


# Instalamos los paquetes de Bioconductor:
...
# Instalamos "SE" para organizar datos de expresión y metadatos; y "POMA" para
# el análisis exploratorio y la normalización de datos:
...
# todos los archivos (data: datasets; scripts: código; results: resultados;
# figuras: gráficos generados; tablas; docs: infromes y borradores):
...
# Para terminar, comprobamos que los archivos estén disponibles en "data":
esperados <- c(
  ...
  file.path(path_data, "GSE173839", "GSE173839_ISPY2_AgilentGeneExp_durvaPlusCtr_FFPE_meanCol_geneLevel_n105.txt.gz"),
  file.path(path_data, "GSE173839", "GSE173839_ISPY2_DurvalumabOlaparibArm_biomarkers.csv.gz"),
...
# Finalmente, realizamos una instantánea del entorno con "renv::snapshot()". De
# esta manera, garantizamos que el entorno pueda ser restaurado para replicar el
# análisis.
set.seed(123)
